...
export const DenyLoan = ({ props, setActive, toggle, setChangeButtons }) => {

  const { declineLoan } = useEmpConx();

  const HandleDeny = async () => {
    try {
      await declineLoan(localStorage.getItem('employeeToken'), props.Dui)
      setChangeButtons(2)
      toggle()
    } catch (error) {
      console.log(error)
  ...
  return (
    <div className='h-[30rem] w-[50rem]'>
      ...
      <div className='w-[80%] mx-auto mt-5 flex flex-row justify-center items-center'>
        <button className='text-white rounded-lg border-none outline-none bg-[#455FB9] hover:bg-[#4f6acb] px-[1rem] py-[.5rem] mr-5' onClick={HandleDeny}>Confirmar</button>
